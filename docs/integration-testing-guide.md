...
**手动测试步骤**：
1. 加载模型
2. 通过 UI/调试入口更新 `scale/position`（当前主界面未接入拖拽/捏合手势时，可先用临时按钮/调试面板验证）
3. 确认 `useLive2DPreferences` 写入了对应模型的 transform
4. 关闭并重新打开应用
5. 再次加载模型
6. 验证位置和缩放是否恢复
...
> 现状说明（2026-01-11）：
> - `app/(tabs)/main.tsx` 里 `useLive2D` 仍未集成 preferences repository（代码里已有 TODO）。
> - Android 原生侧已支持 props `scale/position`，但 RN 主界面尚未把用户手势映射到 `scale/position` 更新，因此“拖拽移动模型/捏合缩放模型→持久化”链路尚不完整。
